...
app.get('/api/schemes', (req, res) => {
  const { category, scope, search, occupation, isStudent, isFarmer, age, gender, annualIncome, state, caste, isBPL } = req.query;
  ...
  if (occupation !== undefined || isStudent !== undefined || isFarmer !== undefined || age !== undefined || state !== undefined) {
...
app.post('/api/ocr', async (req, res) => {
  try {
    ...
    const textPart = {
      text: `Analyze this Indian government identity or revenue document (Aadhaar, Income Certificate, Ration Card, PAN, Land Khatauni, Bank Passbook).
Extract all verifiable information into structured JSON:
{
  "documentType": "Aadhaar Card / Income Certificate / Ration Card / Khatauni",
  "name": "Full Name",
  "age": 35,
  "gender": "Male / Female",
  "state": "State Name",
  ...
  "confidenceScore": 95
    ...
    try {
      ...
      extractedData = { error: 'Failed to parse extracted text' };
    }
