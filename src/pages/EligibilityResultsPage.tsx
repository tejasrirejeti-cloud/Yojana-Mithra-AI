...
export const EligibilityResultsPage: React.FC<EligibilityResultsPageProps> = ({
  ...
  const isStudent = Boolean(profile.isStudent) || /student|college|university|school|pupil|scholar|studying|learner/i.test(profile.occupation || '');
  const isFarmer = Boolean(profile.isFarmer) || /farmer|agri|cultivat|rythu|kisan/i.test(profile.occupation || '');
  const isFemale = profile.gender === 'Female';

  const categoryPriority = (category: GovernmentScheme['category']) => {
    if (isStudent && category === 'Education') return 40;
    if (isFarmer && category === 'Agriculture') return 35;
    if (isFemale && category === 'Women & Child') return 25;
    if (profile.hasDisability && category === 'Healthcare') return 20;
    return 0;
  };

  const results = schemes.map(scheme => evaluateSchemeEligibility(profile, scheme))
    .sort((a, b) => {
      const aEligible = a.score >= 40 ? 1 : 0;
      const bEligible = b.score >= 40 ? 1 : 0;
      if (aEligible !== bEligible) return bEligible - aEligible;

      const priorityDifference = categoryPriority(b.scheme.category) - categoryPriority(a.scheme.category);
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {isStudent && (
        <div className="rounded-3xl border border-indigo-200 dark:border-indigo-900/60 bg-gradient-to-r from-indigo-50 to-sky-50 dark:from-indigo-950/40 dark:to-sky-950/30 p-4 flex items-start gap-3">
          <div className="w-10 h-10 rounded-2xl bg-indigo-100 dark:bg-indigo-900/60 text-[#6C5CE7] flex items-center justify-center shrink-0">
            <Sparkles className="w-5 h-5" />
          </div>
          <div>
            <p className="font-extrabold text-slate-900 dark:text-white text-sm">Student-focused eligibility is active</p>
            <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">Education scholarships, fee reimbursement and education-loan support are prioritized for your profile. Your income, caste, state and age are still checked separately.</p>
          </div>
        </div>
      )}
