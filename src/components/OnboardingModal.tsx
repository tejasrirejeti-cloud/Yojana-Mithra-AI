...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({
  ...
  useEffect(() => {
    ...
    setAge(profile.age ?? '');
    setGender(profile.gender === 'Female' ? 'Female' : profile.gender === 'Other' ? 'Other' : 'Male');
    setState(profile.state || '');
    setDistrict(profile.district || '');
    setOccupation(profile.occupation || '');
    setAnnualIncome(profile.annualIncome ?? '');
    setLandholdingAcres(profile.landholdingAcres ?? '');
    setEducation(profile.education || '10th Pass');
    setCategory(profile.caste || 'General');
    setHasDisability(Boolean(profile.hasDisability));
    setSelectedLanguage(currentLanguage);
  }, [isOpen, profile, currentLanguage]);
