      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 h-16 flex items-center justify-between gap-2 sm:gap-4 overflow-visible">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
          ...
          {user ? (
            ...
            <button
              onClick={() => { if (onGoogleSignIn) void onGoogleSignIn(); else void loginWithGoogle(); }}
            ...
            >
              {isGoogleSigningIn ? <span className="w-3.5 h-3.5 rounded-full border-2 border-[#6C5CE7] border-t-transparent animate-spin" /> : <LogIn className="w-3.5 h-3.5 text-[#6C5CE7]" />}
              <span className="hidden sm:inline">{isGoogleSigningIn ? 'Signing In...' : 'Sign In'}</span>
            </button>
          )}
