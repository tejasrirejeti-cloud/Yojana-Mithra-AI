...
import { CitizenProfile, Language, GovernmentScheme, ChatMessage } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      setCurrentUser(user);
      if (user) {
        // Load user profile from Firestore database
        const dbProfile = await getUserProfileFromFirestore(user.uid);
        if (dbProfile) {
          setProfile(dbProfile);
        } else {
          // Initialize user doc in Firestore
          saveUserProfileToFirestore(user.uid, profile, user.email || undefined);
        }

        // Load saved schemes from Firestore
        const dbSaved = await getSavedSchemesFromFirestore(user.uid);
        if (dbSaved && dbSaved.length > 0) {
          setSavedSchemeIds(dbSaved.map(s => s.schemeId));
        }
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      saveUserProfileToFirestore(currentUser.uid, profile, currentUser.email || undefined);
    }
  }, [profile, currentUser]);
  ...
  const handleSendMessage = async (userText: string) => {
    ...
    try {
      ...
      if (currentUser) {
        saveChatMessageToFirestore(currentUser.uid, 'assistant', aiText);
      }
    } catch (err) {
